package com.example.tpmusic.Fragment;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import com.example.tpmusic.Activity.PlayMusicActivity;
import com.example.tpmusic.Adapter.PlayMusicAdapter;
import com.example.tpmusic.R;

public class FragmentPlayDanhSachBaiHat extends Fragment {
    View view;
    RecyclerView rvdanhsachplaymusic;
    PlayMusicAdapter playMusicAdapter;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_playdanhsachbaihat, container,false);
        rvdanhsachplaymusic = view.findViewById(R.id.rvdanhsachplaymusic);

        if (PlayMusicActivity.mangbaihat.size() >0){
            playMusicAdapter = new PlayMusicAdapter(getActivity(), PlayMusicActivity.mangbaihat);
            rvdanhsachplaymusic.setLayoutManager(new LinearLayoutManager(getActivity()));
            rvdanhsachplaymusic.setAdapter(playMusicAdapter);
        }
        return view;
    }
}
